---
layout: post
title:  "UFC Bot PoC. Step 2. Pinecone + Langchain + GPT3."
...
As mentioned in previous post we decided to use data from Wikipedia. Here in big part we followed [OpenAI cookbook](https://github.com/openai/openai-cookbook/){:target="_blank"}{:rel="noopener noreferrer"} to download the data. We quickly realized that most interesting data is stored in tables, which are not extracted by Wikipedia python package :( So we needed to adjust the script accordingly.

Finally we pulled data from Wikipedia, put it into MySQL. Then generated embeddings and stored them in  Pinecone, including database index id into metadata.
